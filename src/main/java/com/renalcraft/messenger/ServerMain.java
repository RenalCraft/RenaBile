...
public class ServerMain extends WebSocketServer {
    ...
    private String hashPassword(String pass) {
        try {
            if (pass.length() == 64 && pass.matches("^[0-9a-fA-F]+$")) return pass;
    ...
    private void handleRegister(WebSocket conn, JSONObject data) throws Exception {
        ...
        String pass = hashPassword(data.optString("password").trim());
    ...
    private void handleAuth(WebSocket conn, JSONObject data) throws Exception {
        ...
        try (Connection db = getConnection()) {
            ...
            if (rs.next() && rs.getString("password").equals(pass)) {
                ...
                conn.send(new JSONObject().put("type", "ERROR").put("data", new JSONObject().put("message", "Ошибка входа")).toString());
            }
        }
    }
...
private void handleAddFriend(WebSocket conn, JSONObject data) throws Exception {
    ...
    try (Connection db = getConnection()) {
        ...
        if (!rs.next()) return; // Если код не найден, выходим
        String friendUsername = rs.getString("username");

        if (myUsername.equals(friendUsername)) return; // Нельзя добавить самого себя

        // Добавляем связи в таблицу друзей (в обе стороны для взаимности)
        ...
        // Обновляем список друзей у того, кто добавлял
        sendFriendsList(conn, myUsername);

        // Отправляем историю сообщений с этим кодом
        sendRoomHistory(conn, friendCode);

        // Если друг сейчас онлайн, обновляем список друзей и у него
        ...
        if (targetSocket != null && targetSocket.isOpen()) {
            ...
            // Получаем код инициатора, чтобы обновить комнату у друга
